"""
Function for training NRI model.
...
import models.dnri.utils.misc as misc
...
def train(model, train_data, val_data, params, train_writer, val_writer):
    ...
    # setting checkpointing directory
    ...
    for epoch in range(start_epoch, num_epochs+1):
        print("EPOCH", epoch, (end-start))  # time for previous epoch
        model.train()
        ...
        for batch_ind, batch in enumerate(train_data_loader):
            ...
            if verbose:
                print("\tBATCH %d OF %d: %f, %f, %f" % (batch_ind+1, len(train_data_loader), loss.item(),
                                                        loss_nll.mean().item(), loss_kl.mean().item()))

            # number of batches to accumulate the gradient before taking a gradient step
        ...
        # computing ELBO on validation data
        total_nll = 0
        total_kl = 0
        if verbose:
        ...
        with torch.no_grad():
            for batch_ind, batch in enumerate(val_data_loader):
                ...
                if gpu:
                    inputs = inputs.cuda(non_blocking=True)
                loss, loss_nll, loss_kl, logits, _ = model.calculate_loss(inputs, is_train=False,
